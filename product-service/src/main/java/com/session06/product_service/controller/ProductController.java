...
public class ProductController {
    ...
    /**
     * Saga Step 2: Giữ/Trừ tồn kho (Reserve Stock)
     */
    @PostMapping("/reserve-stock")
    public ResponseEntity<StockOperationResponse> reserveStock(@RequestBody List<StockReservationItem> items) {
        try {
            StockOperationResponse response = productService.reserveStock(items);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(StockOperationResponse.builder().success(false).message(e.getMessage()).build());
        }
    }

    /**
     * Saga Compensating Action: Hoàn trả tồn kho (Release Stock)
     */
    @PostMapping("/release-stock")
    public ResponseEntity<StockOperationResponse> releaseStock(@RequestBody List<StockReservationItem> items) {
        StockOperationResponse response = productService.releaseStock(items);
        return ResponseEntity.ok(response);
    }
